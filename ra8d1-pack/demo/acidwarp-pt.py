# acidwarp-pt.py
#
# Acidwarp-style displayio demo for the Renesas EK-RA8D1
# (480x854 MIPI-DSI, ILI9806E panel, Cortex-M85 @ 480 MHz, 64 MB SDRAM).
#
# Same trick the original DOS Acidwarp used: draw a static interference
# pattern ONCE as palette indices, then animate by cycling the palette.
# The bitmap is never rewritten after setup, so per-frame cost is 32 palette
# writes and the panel push, not pixel work.
#
# Exercises the parts of displayio that matter on this board:
#   - a real Bitmap + Palette blitted through a scaled Group
#   - animation by palette rotation
#   - vectorio shapes composited over the bitmap
#   - live text from the builtin font
#
# --- two things learned the hard way on this board, keep them ---
#
# 1. THROTTLE THE LOOP. The panel pushes ~3 fps. An unthrottled animation
#    loop re-dirties the framebuffer faster than a refresh can complete and
#    you get a blank white panel with no error at all. FRAME_DELAY below is
#    load-bearing, not cosmetic.
#
# 2. NO adafruit_display_text on this port. Text is a TileGrid over
#    FONT.bitmap (technique from the pack's dashboard). Also note `import
#    board` works here even though autogen_board_info.toml says otherwise.
#
# Push it with:  RA8_CONSOLE=/dev/cu.usbmodemXXXX python3 tools/putfile.py \
#                  demo/acidwarp-pt.py /code.py
# USB MSC does not work on this board, so there is no CIRCUITPY drive.
