...
/* Vendored from https://github.com/superzazu/z80 @ d64fe10, MIT (LICENSE.z80).
 *
 * Local change: the four fprintf(stderr) diagnostics and z80_debug_output()
 * are the only host dependencies in this file. Building with Z80_TRACE
 * defined (the host harness does) keeps them; the freestanding firmware
 * build leaves it undefined and pulls in no stdio at all. Nothing else is
 * modified, so the zex results carry over to the on-target copy verbatim.
 */
#ifdef Z80_TRACE
#include <stdio.h>
#define Z80_WARN(...) fprintf(stderr, __VA_ARGS__)
#else
#define Z80_WARN(...) ((void)0)
#endif
...
static inline void process_interrupts(z80* const z) {
  ...
  if (z->int_pending && z->iff1) {
    ...
    default:
      Z80_WARN( "unsupported interrupt mode %d\n", z->interrupt_mode);
      break;
    }
...
void exec_opcode(z80* const z, uint8_t opcode) {
  ...
  default: Z80_WARN( "unknown opcode %02X\n", opcode); break;
  }
}
...
void exec_opcode_dcb(z80* const z, uint8_t opcode, uint16_t addr) {
  ...
  default: Z80_WARN( "unknown XYCB opcode: %02X\n", opcode); break;
  }
...
void exec_opcode_ed(z80* const z, uint8_t opcode) {
  ...
  default: Z80_WARN( "unknown ED opcode: %02X\n", opcode); break;
  }
}
