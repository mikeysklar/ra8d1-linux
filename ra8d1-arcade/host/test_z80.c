...
static int run_test(
  ...
  printf("*** %.2f s wall, %.2f M z80-instr/s, %.2f M z80-cycles/s\n\n",
      secs, nb_instructions / secs / 1e6, z->cyc / secs / 1e6);
...
int main(void) {
  ...
  int r = 0;
  r += run_test(&cpu, "zexroms/prelim.com", 8721LU);
  r += run_test(&cpu, "zexroms/zexdoc.cim", 46734978649LU);
  r += run_test(&cpu, "zexroms/zexall.cim", 46734978649LU);
